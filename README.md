...
- Gemini API for structured curriculum auditing
...
- `app/api/ingest/route.ts` - ESP32/browser ingest, Supabase insertion, Tavily grounding, Gemini audit, JWT generation, optional Render deploy.
...
3. Fill in Supabase, Tavily, Gemini, and optional Render credentials in `.env`.
...
- Keep `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_JWT_SECRET`, `GEMINI_API_KEY`, `TAVILY_API_KEY`, and `RENDER_API_KEY` server-only.
